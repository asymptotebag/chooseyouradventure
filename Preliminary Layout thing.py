...
class Foyer(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="Insert hint here").pack(side="top", fill="x", pady=10)
...
class Kitchen(tk.Frame):
    def __init__(self, master):
        ...
        tk.Button(self, text="Garden",
                  command=lambda: master.switch_frame(Garden)).grid(row=9, column = 0, columnspan = 5, sticky = "WE")
        tk.Button(self, text="Oven",
                  command=lambda: master.switch_frame(Oven)).grid(row=9, column = 0, columnspan = 5, sticky = "WE")
